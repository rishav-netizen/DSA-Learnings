...
void Array::DuplicatesWithCount() // O(n^2) 
{
    int count = 0;
    for (int i = 0; i < length - 1; i++)
    {
        ...
        for (int j = i + 1; j < length; j++)
        {
            ...
            {
                count++;
                A[j] = -1; // the duplicates which we have found already we are gonna mark them so we dont count again
            }
        }
        if (count > 1)
        {
            cout << A[i] << " found " << count << " times\n";
        }
    }
